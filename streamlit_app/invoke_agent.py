...
# Initialize SSM client with the specified region
ssm_client = boto3.client('ssm', region_name=theRegion)

# Retrieve agentId and agentAliasId from SSM Parameter Store
def get_ssm_parameter(parameter_name):
    response = ssm_client.get_parameter(Name=parameter_name)
    return response['Parameter']['Value']

try:
    agentId = get_ssm_parameter('/bedrock/agent-id')
    agentAliasId = get_ssm_parameter('/bedrock/agent-alias-id')
except Exception as e:
    print(f"Failed to retrieve parameters from SSM: {str(e)}")
